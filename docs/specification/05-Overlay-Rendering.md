...
- **Vertical video** (ADR-0021): the font scale uses the container's **shorter side** against the 720 px baseline (a 9:16 short is sized by its width, a 16:9 player by its height), so the same style fits both. The extension also rewraps lines at 24 characters and lifts captions to 22 % from the bottom on vertical video.
