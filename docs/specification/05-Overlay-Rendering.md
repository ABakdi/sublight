...
- **Word by word** (built 2026-09-26, default on in the Player and the extension): `revealByWords` expands each cue into steps whose text grows one word at a time, each step starting when its word is spoken, so captions appear exactly as the speech goes. Toggle: Player toolbar "Word by word"; extension popup chip.
