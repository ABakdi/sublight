...
| N6 | Editing *platform* (multi-user, teams, server sync) | Local-first single-user tool focused on subtitles-for-language. |
...
| N8 | Auto-transcribing podcasts/audiobooks from the *file system* | Player handles user-opened files only; no filesystem scanning. |
| N9 | YouTube *downloads* / video saving | yt-dlp is used for **audio** only (optional toggle), never storage of video. |
