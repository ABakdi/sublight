...
- **Python engine (FastAPI + faster-whisper + NLLB)** — strongest raw ASR ecosystem; rejected for env/ops complexity and language split. Might return as a *sidecar* if a specific model (e.g. whisperX alignment) proves necessary.
